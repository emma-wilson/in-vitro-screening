---
title: "Machine Learning: Corrected"
author: "Emma Wilson"
...
projectName <- "InVitro_Corrected_2022_03_31"
...
# Read in erorr correction
errorcorrection <- read.csv(paste0(fileFolders$InputFolder, "errorcorrection_090421.csv"), stringsAsFactors = F)[,c("Title", "Abstract", "Decision", "Keywords")] %>%
  rename(Inclusion = Decision,
         ID = Keywords) %>%
  mutate(cat="Train")

# Reformat data
errorcorrection$Inclusion <- sub("Included", "1", errorcorrection$Inclusion)
errorcorrection$Inclusion <- sub("Excluded", "-1", errorcorrection$Inclusion)

# Make integer
errorcorrection$Inclusion <- as.integer(errorcorrection$Inclusion)

# Subset data
sourceData_minuserror <- sourceData %>%
  filter(!ID %in% errorcorrection$ID)

# Combine corrected data
sourceData <- rbind(sourceData_minuserror, errorcorrection)
```

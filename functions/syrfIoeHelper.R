...
CalculateDecisionForMyStudies <- function(myStudies, NumberScreened, AbsoluteAgreementRatio){
  # include: 1, exclude: 0, unknown: 99
  return( 
    ifelse(myStudies$ScreeningInfo$AgreementMeasure$NumberScreened < NumberScreened |
             myStudies$ScreeningInfo$AgreementMeasure$AbsoluteAgreementRatio < AbsoluteAgreementRatio
           , 99
           , ifelse(myStudies$ScreeningInfo$Inclusion > 0.5, 1, 0))
    # ifelse(numberScreened < 2, 99, ifelse(includedCount >= 2, 1, ifelse((numberScreened-includedCount)>=2, 0,99)))
  )
...
CreateFileFolders <- function(inputFolder = "data/", outputFolder = "output/"){
  ...
  dir.create(df$InputFolder, showWarnings = F, recursive = T)
  dir.create(df$OutputFolder, showWarnings = F, recursive = T)
...
WriteFilesForSyRF <-
  function(myStudies, outputFilenameResultData, myData, threshold, outputFilenameSyRF, outputType ="Hybrid"){
    ...
    resultData[, "included"] = resultData$included

    outputData <- resultData[, c(
      "Title" ,"First Author First Name"  , "First Author Surname" ,"Corresponding Author First Name" ,"Corresponding Author Surname"  ,"Corresponding Author Order"   , "Publication Name"    , "Alternate Name"  , "Abstract"   ,"Url"  ,"Author Address" , "Year"  ,"DOI" , "Reference Type"  , "PDF Relative Path"     , "Keywords"  , "included"
    )]
...
WriteFilesForSyRFErrorAnalysis <-
  function(myStudies, outputFilenameResultData, myData, threshold, outputFilenameSyRF, numberDownload = 200, noRepeat = NULL){
  
    resultData <- ReformDataForSyRF(myStudies, outputFilenameResultData, myData, threshold)
    
    if(!is.null(noRepeat)) {
      resultData <- resultData[!(resultData$Keywords %in% noRepeat$OldStudyIdStr),]
    }
    
    resultData$Id <- rownames(resultData)
    
    resultData <- resultData %>%
      mutate(absScore = ifelse(score > threshold,  abs(score - threshold)/(1-threshold), abs(score - threshold)/threshold))
    ...
    resultData <- resultData %>%
      ...
      mutate( included = ifelse(Id %in% resultDataChange$Id, includedML, includedHM))
    
    outputData <- resultData %>% 
      filter(includedHM != included) %>% 
      select(Title, Authors = "First Author Surname","Publication Name","Alternate Name", "Abstract", "Url","Author Address", "Year","DOI",  "Keywords", "Reference Type" ,"PDF Relative Path",   "includedHM","included" )
    
    outputData$Title <- gsub('[\"]',"'",outputData$Title)
    outputData$Abstract <- gsub('[\"]',"'",outputData$Abstract)
...
PlotPerformance <- function(analysisResult, graphnames,projectName){
  # analysisResult <- analysisResult %>%
  #   gather("Type", "Performance", -c(Chosen, Thresholds))
  chosenThresholds <- analysisResult$Thresholds[which(analysisResult$Chosen)][1]
  
  myPlot <- ggplot(data=analysisResult, aes(x=FPR, y=Sensitivity
                                            # , group=Type
                                            # , colour=Type
                                            , title = paste0(projectName, " ROC"))) + geom_line() + geom_vline(xintercept=chosenThresholds )+ ggtitle(projectName)
